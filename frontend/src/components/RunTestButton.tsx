...
export function RunTestButton({
  ...
  const [useHttpCredentials, setUseHttpCredentials] = useState(false);
  ...
  const handleRunTest = async (withProfile: boolean = false) => {
    ...
    try {
      ...
      const response = await executionService.startExecution(testCaseId, {
        ...
        http_credentials: useHttpCredentials && httpUsername && httpPassword
          ? {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  type="checkbox"
                  checked={useHttpCredentials}
                  onChange={(e) => setUseHttpCredentials(e.target.checked)}
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                Use HTTP Basic Auth (Optional)
              </label>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Use this for environments that require Basic Auth before loading the site.
              </p>
              {useHttpCredentials && (
