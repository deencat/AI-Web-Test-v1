...
**Phase 2 (Weeks 9-14) - Learning Foundations (Q1 2026):**
- 📋 Test case editing and versioning
- 📋 Execution feedback collection
- 📋 Pattern recognition and auto-fix suggestions
- 📋 KB-enhanced test generation with learned patterns
- 📋 Learning insights dashboard
- 📋 Prompt template library and A/B testing
- 📋 Optional: Simple ML models (CPU-based)

**Phase 3 (Weeks 15-26) - Multi-Agent Architecture + Enterprise Integration (Q2 2026):**
- 📋 Multi-agent architecture (6 specialized agents)
- 📋 Observation Agent (real-time monitoring with ML)
- 📋 Requirements Agent (PRD analysis)
- 📋 Analysis Agent (root cause analysis)
- 📋 Evolution Agent (self-healing tests)
- 📋 Agent orchestration with message bus
- 📋 CI/CD integration (Jenkins, GitHub Actions)
- 📋 JIRA integration for defect tracking
- 📋 Production monitoring (Prometheus, Grafana)
- 📋 Advanced KB features (full-text search, versioning)

**Phase 4 (Weeks 27-34) - Reinforcement Learning (Q2 2026):**
...
- **Learning Foundations (Phase 2)**: Test editing, feedback loops, pattern recognition for immediate productivity gains
- **Multi-Agent Architecture (Phase 3)**: Six specialized AI agents working autonomously
- **Self-Healing Tests (Phase 3)**: Automatic test repair when UI changes
- **Reinforcement Learning (Phase 4)**: Continuous learning from test results and production incidents
- **Advanced Analytics (Phase 3)**: Root cause analysis, pattern recognition, and predictive insights
...
**FR-08: Advanced Test Execution (📋 Phase 3)**
...
### 3.2 Learning Foundations (📋 PLANNED FOR PHASE 2 - Weeks 9-14)

**Current Implementation Note**: Phase 1 MVP uses direct LLM API calls for test generation. Phase 2 will add learning mechanisms and feedback loops to improve test quality without complex multi-agent architecture.

**FR-10: Test Case Editing & Versioning (📋 Phase 2)**
- Inline step editor for modifying test steps without full regeneration
- Version control with full change history
- Rollback capability to any previous version
- Track changes by user or AI
- Compare versions and view diffs

**FR-11: Execution Feedback Collection (📋 Phase 2)**
- Automatic capture of detailed failure context
- Store failure_type, error_message, screenshots, HTML snapshots
- Track human corrections and what fixed failures
- Performance metrics (step duration, memory usage)
- Anomaly detection (statistical-based)

**FR-12: Pattern Recognition & Auto-Fix (📋 Phase 2)**
- CPU-based pattern analyzer for failure analysis
- Statistical matching of similar past failures
- Auto-suggest fixes based on successful corrections
- High-confidence fixes (>85%) applied automatically
- Lower confidence (60-85%) suggested to users

**FR-13: KB-Enhanced Test Generation (📋 Phase 2)**
- Expand KB with learned patterns categories:
  - test_patterns: Successful test structures
  - failure_lessons: Common mistakes to avoid
  - selector_library: Working selectors by domain
- Auto-populate KB from successful executions
- Integrate learned patterns into generation prompts

**FR-14: Learning Insights Dashboard (📋 Phase 2)**
- Display key learning metrics and visualizations
- Most common failures with suggested fixes
- Success rate trends over time
- Learned patterns library with usage stats
- Suggested test improvements with confidence scores

**FR-15: Prompt Template Library & A/B Testing (📋 Phase 2)**
- Manage multiple prompt template variants
- CPU-based A/B testing infrastructure
- Automatic performance tracking per template
- Auto-deactivate underperforming prompts (<60% success)
- Data-driven prompt optimization

**FR-16: Optional ML Models (📋 Phase 2 - Bonus)**
...
### 3.3 Multi-Agent Architecture (📋 PLANNED FOR PHASE 3 - Weeks 15-26)

**Implementation Note**: Multi-agent architecture delayed to Phase 3 after Phase 2 Learning Foundations prove ROI and collect quality training data.

**FR-17: Autonomous Agent System (📋 Phase 3)**
- Six specialized AI agents working in coordination
- Agent-to-agent communication via message bus (Redis Streams)
...
**FR-18: Requirements Agent (📋 Phase 3)**
...
**FR-19: Generation Agent (📋 Phase 3)**
...
**FR-21: Observation Agent (📋 Phase 3)**
- Real-time monitoring of test execution with ML-based anomaly detection
- Performance metric collection and analysis
- Screenshot and video capture on failures
- Log aggregation and correlation
- 85-90% anomaly detection accuracy

**FR-22: Analysis Agent (📋 Phase 3)**
...
**FR-23: Evolution Agent (📋 Phase 3)**
- Rule-based self-healing tests (85%+ success rate)
...
**FR-24: Agent Orchestration & Coordination (📋 Phase 3)**
...
**FR-25: Knowledge Base with Categorization (✅ IMPLEMENTED in Phase 1)**
...
### 3.19 Saga Pattern for Distributed Workflows

**FR-75: Saga Pattern for Distributed Workflows**
- Temporal.io 1.22.0 workflow orchestration for 6-agent distributed workflows (Requirements Agent → Generation Agent → Execution Agent → Observation Agent → Analysis Agent → Evolution Agent) with durable execution (workflows survive crashes, resume from last checkpoint, no work lost)
- Compensation logic for backward recovery on failures: requirements_agent_compensate (delete scenarios from database, idempotent), generation_agent_compensate (mark tests as invalid for audit trail instead of deletion, preserve data lineage), execution_agent_compensate (optional environment cleanup for failed test runs)
- Retry policies with exponential backoff per agent: Requirements Agent (3 attempts, initial 1s → max 30s, backoff 2.0x), Generation Agent (3 attempts, initial 2s → max 60s), Execution Agent (2 attempts, initial 5s → max 2 min for long-running tasks, heartbeat timeout 2 min to monitor progress), non-retryable errors (ValidationError, AuthenticationError)
- Workflow patterns: Sequential saga (main pattern for Requirements → Generation → Execution → Observation → Analysis → Evolution in order), Parallel saga (execute multiple tests concurrently with asyncio.gather), Child workflows (orchestrate multi-project workflows with start_child_workflow), Long-running saga (user control with signals for cancel/approve/set_priority and queries for get_status)
- State management: Automatic workflow state persistence in Temporal PostgreSQL database (separate from application database), workflow state includes completed_steps list + failed_steps + retry_count, state survives crashes and system restarts, workflow versioning for backward compatibility during updates
- Temporal cluster architecture: 3-node high availability deployment in Kubernetes (Frontend service API gateway, History service event store with complete audit trail, Matching service task routing), PostgreSQL 15+ backend for workflow state + event history + task queues, Temporal Web UI port 8080 for workflow monitoring (view running, inspect history, retry failed, cancel running)
- Temporal workers: Python SDK temporal-sdk 1.5.0 integrated in backend pods (no additional infrastructure), workers execute workflows (saga orchestration logic) and activities (agent invocations), auto-scaling based on workflow queue depth (scale up when queue >100, scale down when queue <10)
- Timeout management: start_to_close_timeout (Requirements 5 min, Generation 10 min, Execution 30 min for complete activity execution), schedule_to_start_timeout 1 min (queue time before activity starts), heartbeat_timeout 2 min (for long-running Execution Agent to send progress updates), schedule_to_close_timeout (total time including all retries)
- Monitoring & observability: Prometheus metrics (temporal_workflow_started_total counter, temporal_workflow_completed_total by status, temporal_workflow_duration_seconds histogram with p50/p95/p99, temporal_compensation_executed_total by activity type), Grafana dashboard (8 panels: workflow success rate %, active workflows gauge, duration p95 graph, compensation rate counter, activity failures by type, queue depth gauge, retry rate, event sourcing audit trail)
- Event sourcing: Complete workflow history stored in Temporal database (all steps, decisions, activities, compensations, retries with timestamps), enables workflow replay for debugging, provides compliance audit trail, supports workflow versioning and migration

### 3.20 Playwright Test Agents Integration

**FR-76: Markdown Test Specifications**
- Requirements Agent outputs human-readable Markdown specs in `specs/` directory with Playwright-style structured format (Application Overview describing target app features/environment/browsers, Test Scenarios with numbered subsections 1.1/1.2/etc, Steps and Expected Results for each scenario with specific assertions, Data Requirements for test users/databases/mocks, Success Criteria for pass conditions)
- Version-controlled in Git for traceability and stakeholder review (commit specs with PRD references, PR reviews with inline comments, meaningful diffs showing scenario changes, blame tracking for scenario ownership)
- Generation Agent reads from Markdown specs instead of JSON for test code generation (LLM-powered parser extracts structured scenarios, maps steps to Playwright actions, preserves seed test references, generates test files with spec path comments)
- Inline seed test references: `Seed: tests/three-hk/seed.spec.ts` for environment context and setup dependencies
- Automatic spec generation from natural language requirements with GPT-4/Claude (structured prompt with examples, scenario grouping by user flow, edge case identification, expected results inference)
- Spec validation before test generation: completeness check (all required sections present), format validation (Markdown syntax correct, heading hierarchy valid), scenario ID uniqueness (no duplicate 1.1, 1.2 IDs)
- Spec template library for common patterns: login/logout flows, CRUD operations, form submissions, API integrations, error handling scenarios

**FR-77: Seed Test Pattern**
- `tests/*/seed.spec.ts` provides environment setup for all tests in project (navigation to base URL, authentication with test credentials, cookie acceptance banner handling, initial data fixture setup, environment-specific configuration)
- Referenced in Markdown test specifications via `Seed: tests/three-hk/seed.spec.ts` comment for clarity
- Execution Agent runs seed tests first in dependency order before executing dependent tests in workflow (verify seed success before proceeding, skip dependent tests if seed fails with clear reason, log seed execution time and success rate)
- Storage state saved to `tests/*/.auth/*.json` for reuse across tests (authenticated sessions with JWT tokens, cookies including session and tracking, local storage preferences, IndexedDB data if needed)
- Seed test failure handling: skip all dependent tests, log detailed failure reason (authentication failed, navigation timeout, fixture setup error), notify QA team via dashboard alert, create incident report for investigation
- Code reduction target: **50%+ elimination of duplicated setup code** (100 tests × 50 lines = 5,000 lines saved), faster test execution: 30-50% improvement from storage state reuse, easier maintenance: single point of change for setup logic

**FR-78: Healer-Enhanced Self-Healing** 🔥 **HIGH PRIORITY**
- Evolution Agent implements replay → inspect → patch → verify workflow for failing tests (4-phase Healer pattern from Playwright Test Agents, production-ready implementation)
- **Phase 1 Replay**: Execute test step-by-step to reproduce failure (use Playwright to re-run each action, stop at first exception, capture exact failing step index, classify failure type from exception message), failure types: element_not_found (selector invalid or element removed), timeout (page/action took >30s), element_detached (DOM re-rendered), click_intercepted (overlay or z-index issue), navigation_failed (URL not loaded)
- **Phase 2 Inspect**: Navigate to failing point and inspect CURRENT UI state (execute all steps up to but not including failing step, capture DOM snapshot at failure point, analyze visible elements on page, find similar elements via multiple strategies), similarity strategies ranked by confidence: 1) find_by_text (match visible text), 2) find_by_role (ARIA roles), 3) find_by_label (form labels), 4) find_by_placeholder (input placeholders), 5) find_by_test_id (data-testid attributes), similarity scoring: 0.0-1.0 based on text/attribute matching
- **Phase 3 Patch**: Generate patch candidates based on failure type and inspection data (for element_not_found: selector_update patches using similar elements with similarity_score >0.7, multiple candidates ranked by score, reason: "Found similar element via find_by_role"; add_wait patches for explicit waits 5s or networkidle, confidence 0.6-0.7, reason: "Element may need time to appear"), (for timeout: increase_timeout from 30s to 60s, confidence 0.8, add_wait for networkidle, confidence 0.7), (for click_intercepted: scroll_into_view patch confidence 0.85, force_click with force:true option confidence 0.7), (for element_detached: requery_element before each action, confidence 0.8), sort patches by confidence score descending
- **Phase 4 Verify**: Apply patch and verify fix (update test code with patch changes, re-run entire test in clean environment, if status="pass": commit patch permanently + log success with confidence score + update test in database, if status="fail": revert patch changes immediately + try next patch candidate + log attempt with reason), exhaust all candidates: skip test + create detailed Healer report (diagnosis with inspection data, attempted patches list with descriptions/reasons/results, recommendation for manual review, timestamp and test_id) + alert QA team via Slack/dashboard
- Target metrics: **98%+ self-healing success rate** (up from 95%, +3.2% improvement), average repair time 45 seconds (includes replay + inspect + patch + verify steps), manual intervention rate <2% (down from 5%, -60% reduction), 100% of healed tests have verification pass before commit
- Healer reports stored in database: test_id, failure_type, inspection_data JSON, attempted_patches array, final_status (healed/unable_to_heal/skipped), created_at timestamp, for compliance audit trail and continuous improvement analysis
- Skip test if repair impossible: functionality broken (not test issue), mark test status="skipped" in database, reason="functionality_broken", detailed report includes: "Manual review required. Healer attempted X fixes but none worked. Application functionality may have changed."

**FR-79: Web-Based Interactive Test Planning**
- Web dashboard component `/planner` for interactive test plan creation (embedded Playwright browser showing target application in iframe, real-time Markdown spec preview panel, planner controls for start/pause/stop exploration)
- QA engineer navigates and interacts with live application while Requirements Agent observes and generates scenarios (enter target URL + project name, click Start Exploration button, embedded browser loads target app, QA clicks elements/fills forms/navigates naturally, agent tracks all interactions in real-time)
- Real-time interaction tracking via Playwright page events: clicks (element selector, visible text, timestamp), inputs (field selector, input value, field type), navigation (URL changes, page title, timestamp), hovers (element detection for intent inference), form submissions (form data captured)
- Live Markdown spec generation as engineer explores (Application Overview auto-populated from page title/meta, Test Scenarios generated from interaction patterns, Steps numbered sequentially from observed actions, Expected Results inferred from page state changes, Edge Cases suggested by agent based on similar elements/validation rules)
- WebSocket-based communication at `/ws/planner/{session_id}` for real-time updates (bi-directional: frontend sends commands navigate/get_interactions/generate_spec, backend sends events navigation/interactions/spec_generated, reconnection handling on disconnect, JSON message protocol)
- Interactive features: Add Note button to annotate interactions, Mark Scenario button to create explicit scenario boundaries, Copy Markdown button to copy spec to clipboard, Edit button to refine spec before generation, Save button to save spec to specs/ directory
- One-click test generation: Generate Tests button → reads completed Markdown spec → invokes Generation Agent → creates executable tests in tests/ → shows success confirmation
- Performance targets: **50% faster test planning** (10 hours/week → 5 hours/week per QA engineer), **40% more accurate scenarios** (based on actual app state vs document guessing), 30% more edge cases discovered (agent observes element states/validation patterns)
- **Note**: Web-based only via dashboard at /planner endpoint, no IDE integration (per requirement for standalone tool), works on any browser accessing dashboard, no VS Code/CLI dependencies
