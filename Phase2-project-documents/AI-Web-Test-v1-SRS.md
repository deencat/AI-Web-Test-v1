...
- **Phase 2 (Weeks 9-14) - Learning Foundations:**
  ...
  - Pattern recognition and auto-fix
  - KB-enhanced generation
  - Learning insights dashboard
  - Prompt A/B testing
- **Phase 3 (Weeks 15-26) - Multi-Agent Architecture + Enterprise:**
  - Multi-agent architecture with orchestration
  - Observation, Requirements, Analysis, Evolution agents
  - CI/CD integration (Jenkins, GitHub Actions)
  - JIRA integration
  - Production monitoring (Prometheus, Grafana)
  - Advanced KB features (full-text search, versioning)
- **Phase 4 (Weeks 27-34) - Reinforcement Learning:**
...
> 
> This SRS describes the current Phase 1 implementation and the phased roadmap for future enhancements.
> 
> - **✅ Implemented** = Currently working in Phase 1 MVP
> - **📋 Phase 2** = Learning Foundations (Weeks 9-14)
> - **📋 Phase 3** = Multi-Agent Architecture + Enterprise (Weeks 15-26)
> - **📋 Phase 4** = Reinforcement Learning (Weeks 27-34)
> 
> See `Phase2-project-documents/AI-Web-Test-v1-Project-Management-Plan-REVISED.md` for complete phased approach.
> 
> **Key Distinction:** Phase 1 uses direct LLM API calls. Phase 2 adds learning mechanisms. Phase 3 introduces full multi-agent orchestration system.
...
- **Purpose:**  
  ...
  **Phase 1 MVP (COMPLETE)** delivers natural language test generation and automated browser testing.
  
  **Phase 2 (Weeks 9-14)** will add learning foundations: test editing, feedback collection, pattern recognition, and KB-enhanced generation for 2-3x productivity improvement.
  
  **Phase 3 (Weeks 15-26)** will introduce a **multi-agent agentic architecture** with coordinated specialized AI agents (Observation, Requirements, Analysis, Evolution) for autonomous, self-learning test automation.
  
  **Phase 4 (Weeks 27-34)** will implement reinforcement learning for continuous improvement.  
...
- **Core Modules:**  
  1. **User Interface Layer:** Dashboard, natural language input, reporting views, KB document upload and management, **debug session UI**, **test editing UI (Phase 2)**, **learning insights dashboard (Phase 2)**, **agent monitoring interface (Phase 3)**
  2. **API Layer:** RESTful APIs for UI-backend communication (Python FastAPI), **7 debug mode endpoints**, **Phase 2 learning endpoints**, **Phase 3 agent APIs**
  3. **Learning Layer (Phase 2):** Test versioning, feedback collection, pattern analyzer, prompt A/B testing
  4. **Agent Orchestration Layer (Phase 3):** Central coordinator managing six specialized AI agents
  5. **Message Bus Layer (Phase 3):** Event-driven communication infrastructure for agent coordination
  6. **AI Agent System (Phase 3):** Six specialized agents (Requirements, Generation, Execution, Observation, Analysis, Evolution)
  7. **Test Execution Engine:** Stagehand-based executor with Playwright/Selenium fallback, **persistent browser debug mode**
  8. **AI Integration Layer:** OpenRouter API client with model management and fallback strategies
  9. **Knowledge Base Layer:** Categorized document repository with metadata management, **learned patterns (Phase 2)**, **full-text search (Phase 3)**, **versioning (Phase 3)**
  10. **Data Layer:** PostgreSQL for structured data (including debug_sessions, test_versions, execution_feedback tables), Redis for caching, Vector DB for agent memory (Phase 3), S3/MinIO for KB documents
  11. **Debug Session Management Layer:** Persistent browser lifecycle, auto/manual mode orchestration, token tracking, iteration history
  12. **Observability Layer:** Prometheus metrics (Phase 3), distributed tracing (Phase 3), centralized logging  

***

### **Architecture Pattern**
- **Phase 1 (Current):** Monolithic FastAPI application with direct LLM calls
- **Phase 2 (Weeks 9-14):** Enhanced monolith with learning mechanisms (test versioning, feedback collection, pattern recognition)
- **Phase 3 (Weeks 15-26):** Multi-Agent Event-Driven Microservice Architecture  
- **Secondary Patterns:** CQRS (Command Query Responsibility Segregation), Saga Pattern for agent workflows (Phase 3)

- **Architectural Layers:**
  - **Presentation Layer:** React with Redux Toolkit (WebSocket for real-time updates in Phase 3)
  - **API Gateway Layer:** FastAPI with REST endpoints
  - **Learning Layer (Phase 2):** Pattern analyzer, feedback collector, version manager, prompt A/B tester
  - **Agent Orchestration Layer (Phase 3):** Central coordinator with agent lifecycle management
  - **Agent Layer (Phase 3):** Six specialized autonomous agents with independent business logic
  - **Message Bus Layer (Phase 3):** Event-driven messaging (Redis Streams) for agent communication
  - **AI Integration Layer:** OpenRouter client with circuit breaker and fallback
  - **Data Persistence Layer:** PostgreSQL (transactional), Redis (cache), Vector DB (agent memory - Phase 3)
  - **Observability Layer (Phase 3):** Prometheus, Grafana, distributed tracing (Jaeger/Zipkin)

- **Multi-Agent Architecture Components (Phase 3):**
...
### **Phase 2: Learning Foundations Architecture (Weeks 9-14)**

**Overview:** Phase 2 enhances the Phase 1 monolithic architecture with learning mechanisms and feedback loops WITHOUT introducing multi-agent complexity.

#### **Core Components**

**1. Test Versioning System**
- **Database Schema:**
  - `test_versions` table with version_number, created_by, change_reason, parent_version_id
  - Full JSON storage of test steps per version
  - Rollback tree for version navigation
- **API Endpoints:**
  - `PUT /api/v1/tests/{id}/steps` - Edit test steps
  - `GET /api/v1/tests/{id}/versions` - Get version history
  - `POST /api/v1/tests/{id}/rollback` - Rollback to version
- **Implementation:** SQLAlchemy models, migration scripts

**2. Execution Feedback Collection**
- **Database Schema:**
  - `execution_feedback` table with failure details, corrections, performance metrics
  - Stores: failure_type, error_message, screenshot_url, page_html_snapshot
  - Tracks: corrected_step, correction_source, correction_confidence
  - Performance: step_duration_ms, memory_usage_mb
  - Anomalies: is_anomaly, anomaly_score, anomaly_type
- **Automatic Capture:** Integrated into test execution service
- **API Endpoints:**
  - `GET /api/v1/feedback/{execution_id}` - Get feedback
  - `POST /api/v1/feedback/{id}/correction` - Submit correction
- **Implementation:** Background task for feedback capture

**3. Pattern Recognition Service**
- **PatternAnalyzer Class (CPU-based):**
  - `analyze_failure_patterns()` - Find similar past failures
  - `extract_common_pattern()` - Statistical pattern matching
  - `detect_anomalies()` - Simple statistical anomaly detection
- **Confidence Scoring:** 0.0-1.0 based on evidence count
- **Auto-Apply Threshold:** >0.85 for automatic fixes
- **Suggestion Threshold:** 0.6-0.85 for user suggestions
- **Implementation:** Python service, no ML libraries required initially
...
- **Auto-Population Service:**
  - Triggered on successful executions (pass_rate > 0.9)
  - Extracts patterns and selectors
  - Stores in KB with domain tagging
- **Enhanced Prompt Template:**
  - Includes learned patterns in system prompt
  - References working selectors by domain
  - Incorporates failure lessons
- **Implementation:** Extended KB service, prompt template updates

**5. Learning Insights Dashboard**
- **API Endpoint:** `GET /api/v1/learning/insights`
- **Metrics Calculated:**
  - Most common failures (failure type, count, success rate)
  - Success rate trend (7, 30, 90 days)
  - Learned patterns library (usage count, success rate)
  - Suggested improvements (test_id, issue, suggestion, confidence)
  - KB statistics (pattern count, selector count, most referenced docs)
- **Visualizations:** Recharts (bar chart, line chart, tables)
- **Implementation:** Backend aggregation queries, React frontend

**6. Prompt Template & A/B Testing**
- **Database Schema:**
  - `prompt_templates` table with performance tracking
  - Fields: total_uses, success_count, success_rate, avg_generation_time_ms, avg_tokens_used
  - A/B testing: is_active, traffic_allocation (0.0-1.0)
- **PromptABTester Class:**
  - `select_prompt_template()` - Weighted random selection
  - `update_performance()` - Running average updates
  - Auto-deactivation: <60% success rate after 100 uses
- **Management UI:** React component for template CRUD
- **Implementation:** Backend service, frontend management page

**7. Optional: Simple ML Models (Bonus)**
- **TestSuccessPredictor:** Logistic regression (scikit-learn)
- **FailureTypePredictor:** Random forest (scikit-learn)
- **Training Schedule:** Weekly on historical data
- **Minimum Data:** 1000+ executions for success predictor, 500+ failures for failure predictor
- **Implementation:** Python scikit-learn, joblib for model persistence

#### **Phase 2 Data Flow**

```
User edits test → Version created → Stored in test_versions
                        ↓
Test executed → Feedback captured → execution_feedback table
                        ↓
Pattern Analyzer → Analyzes failures → Suggests fixes
                        ↓
Successful executions → Auto-populate KB → Learned patterns
                        ↓
Next generation → Uses learned patterns → Improved quality
```

#### **Phase 2 Success Metrics**

- Test editing response time: <200ms
- Feedback collection overhead: <100ms
- Pattern analysis time: <5 seconds
- Auto-fix accuracy: >85% (high-confidence only)
- Generation success rate improvement: 60% → 85% (+25%)
- Manual corrections reduced: 60%
- Test regenerations reduced: 85%

***

### **Phase 3: Multi-Agent System Architecture (Weeks 15-26)**

**Overview:** Phase 3 introduces the full multi-agent architecture after Phase 2 proves ROI and collects quality training data.
...
- Markdown Specifications: Structured test plans in specs/ directory (Playwright-style format with Application Overview, Test Scenarios 1.1/1.2 numbered subsections, Steps and Expected Results, Data Requirements, Success Criteria) stored in Git (version control with meaningful diffs, PR reviews with inline comments, blame tracking for ownership)
- Spec Generator: LLM-powered Markdown generation in Requirements Agent (GPT-4/Claude with structured prompts, scenario grouping by user flow, edge case identification, expected results inference from requirements, spec template library for common patterns login/CRUD/forms/error-handling)
- Spec Parser: Markdown to structured JSON conversion in Generation Agent (extract scenarios with regex + LLM, map steps to Playwright actions, preserve seed test references, generate test files with `// spec: specs/three-hk/login.md` comments)
- Spec Validation: Completeness check (all required sections Application Overview/Scenarios/Success Criteria present), format validation (Markdown syntax via python-markdown, heading hierarchy h1→h2→h3 valid), scenario ID uniqueness check (no duplicate 1.1/1.2 IDs)
- Seed Test Pattern: tests/*/seed.spec.ts convention for environment setup (navigation to base URL, authentication with test credentials, cookie acceptance via getByRole('button', {name: 'Accept'}), fixture data initialization, environment-specific config)
- Seed Test Execution: Execution Agent runs seed tests first (verify seed success before dependent tests, skip dependent tests if seed fails with reason logged, track seed execution time and success rate metrics)
- Storage State: Playwright storageState API for session persistence (authenticated sessions with JWT tokens saved to tests/*/.auth/*.json, cookies for session/tracking, localStorage preferences, IndexedDB data, reuse state via test.use({ storageState: 'path' }))
- Healer Logic (4-Phase): Evolution Agent self-healing workflow (Phase 1 Replay: step-by-step Playwright re-execution + exception capture + failure classification into 5 types element_not_found/timeout/element_detached/click_intercepted/navigation_failed), (Phase 2 Inspect: navigate to failing point + capture DOM snapshot + analyze visible elements + find_similar_elements via 5 strategies text/role/label/placeholder/testid ranked by similarity 0.0-1.0), (Phase 3 Patch: generate patch candidates with confidence scores 0.6-0.85 based on failure type, 7 patch types selector_update/add_wait/increase_timeout/scroll_into_view/force_click/requery_element, sort by confidence descending), (Phase 4 Verify: apply patch + re-run test + if pass commit permanently + if fail revert + try next candidate + exhaust all: skip test + create Healer report + alert QA team)
- Healer Database Schema: healer_reports table (test_id FK, failure_type enum, inspection_data JSONB with DOM snapshot, attempted_patches JSONB array with description/reason/confidence/result, final_status enum healed/unable_to_heal/skipped, created_at timestamp) for audit trail and analysis
- Interactive Planner: WebSocket endpoint /ws/planner/{session_id} for live app exploration (FastAPI WebSocket with async_playwright Chromium browser, page event listeners for click/input/navigation/hover, interaction tracking with element selector + text + timestamp)
- Embedded Browser: Playwright browser in web dashboard iframe (browser context management, storageState persistence for sessions, element inspection APIs locator.boundingBox()/attributes(), screenshot/video capture on demand)
- Live Spec Generation: Real-time Markdown spec from interactions (LLM converts interaction sequence to scenarios, group interactions by user flow patterns, infer steps/expected results from page state changes, suggest edge cases from element states/validation rules)
- WebSocket Protocol: Bi-directional JSON messages (frontend commands: navigate/get_interactions/generate_spec/stop, backend events: navigation with url/title, interactions with data array, spec_generated with markdown content, reconnection handling on disconnect)
- Generation Optimization: Playwright best practices in Generation Agent (generation hints prefer getByRole('button', {name: 'Login'}) over CSS #login-btn, getByLabel('Email') for form inputs, getByText() for visible text, getByTestId() as last resort), assertion catalog 7 categories (toBeVisible/toHaveText/toHaveAttribute/toBeEnabled/toHaveURL/toHaveTitle/toHaveCount with 30+ assertion patterns), live selector verification during generation (test selector on actual page + find alternatives if invalid + fallback to similar elements)
- Selector Strategies: 5 strategies ranked by resilience (1 getByRole ARIA roles most resilient, 2 getByLabel form labels, 3 getByText visible text, 4 getByPlaceholder input hints, 5 getByTestId data-testid as last resort), similarity scoring algorithm (text match cosine similarity, attribute overlap Jaccard index, combined score 0.0-1.0)
- Interactive Planner UI: React component at /planner route (embedded browser iframe with target app, real-time spec preview panel with live Markdown, planner controls start/pause/stop buttons, interaction stats counter, annotation buttons Add Note/Mark Scenario, spec actions Copy Markdown/Edit/Save to specs/)
