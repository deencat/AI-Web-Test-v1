...
**Goal:** A user on the Saved Tests page clicks **Run** on any test and it executes immediately. No browser profile picker. No manual credential entry. UAT credentials are auto-injected. Non-HTTP-auth URLs run as-is.

**Scope boundary:** Agent workflow (`OrchestrationService`, `ObservationAgent`) is **not changed** — it already works correctly.

---

#### Two Gaps Closed by This Sprint

| # | Gap | Current behaviour | Expected behaviour |
|---|-----|-------------------|-------------------|
| **G1** | `ExecutionService` never calls `http_credentials_for_url()` | Saved test against `wwwuat.three.com.hk` fails with 401; user must select a browser profile with manually entered credentials | Backend auto-injects hardcoded UAT creds into Playwright `new_context(http_credentials=...)` — same as agent workflow. Non-UAT URLs run with no credentials (unchanged). |
| **G2** | `RunTestButton` always shows a browser profile picker | User must pick a profile before every run regardless of URL | Browser profile picker removed entirely — for UAT URLs a `🔐 UAT credentials auto-applied` badge is shown; for all other URLs the Run button is shown directly with no picker |

---

#### Design: How the Saved Tests Page Works After This Sprint

```
User on Saved Tests page — test URL is wwwuat.three.com.hk
  → Clicks "Run" (no picker shown)
  → Badge "🔐 UAT credentials auto-applied" displayed below the button
...
User on Saved Tests page — test URL is non-UAT (prod, staging, etc.)
  → Clicks "Run" (no picker shown — profile picker removed for all URLs)
  → POST /v1/executions/{id}/run sent (no browser_profile_id)
  → ExecutionService calls http_credentials_for_url(test.url) → returns None
  → Playwright new_context() with no http_credentials — normal execution
```
...
#### Phase 2: Frontend — Remove Profile Picker from RunTestButton (G2)

**Problem:** `RunTestButton` renders a browser profile `<select>` dropdown for all tests unconditionally, blocking Run until the user makes a selection. This is unnecessary for all URLs: UAT credentials are hardcoded and auto-applied; non-HTTP-auth URLs need no credentials.

**Fix:** Remove the profile picker entirely from `RunTestButton`. For UAT URLs, show a read-only `🔐 UAT credentials auto-applied` info badge below the Run button. For all other URLs, show the Run button directly with no picker and no badge.

| # | Task | File | Details | Risk |
|---|------|------|---------|------|
| 2.1 | **`isUatUrl(url: string): boolean` utility** — hostname check for `wwwuat.three.com.hk` | `frontend/src/utils/urlUtils.ts` | Small pure function | Low |
| 2.2 | **`RunTestButton.tsx` — remove profile picker; add UAT badge** — delete the profile `<select>` element and its associated state (`selectedProfileId`); replace with: `{isUatUrl(test.url) && <span>🔐 UAT credentials auto-applied</span>}` below the Run button | `frontend/src/components/RunTestButton.tsx` | Picker and its state fully removed; `browser_profile_id` no longer sent in payload | Low |
| 2.3 | **Frontend unit tests** — (a) UAT URL: Run button present, badge present, profile picker absent; (b) non-UAT URL: Run button present, badge absent, profile picker absent; (c) `isUatUrl` returns correct values | `frontend/src/components/__tests__/RunTestButton.test.tsx` | Extends existing test file | Low |

**Total Phase 2: 2.5 points / ~1.25 days**
...
**Owner:** Developer B  
**Status:** ✅ **COMPLETE** (Feature A May 13, 2026 · Feature B May 14, 2026)  
**Story Points:** 20 points / 5 days (COMPLETE)  
...
#### Feature B: Re-Run from Failed Step ✅ COMPLETE (May 14, 2026)
...
After each *passing* step, `ExecutionService._save_step_snapshot()` calls `export_profile_session()` and persists the result against `(execution_id, step_number)` in the new `step_session_snapshots` table along with `page_url`. Snapshot is JSON (`{cookies, localStorage, sessionStorage, exported_at}`) — same format used by the existing Persistent Browser Debug Mode. Snapshot save is non-fatal (exceptions are caught and logged; never interrupts execution).

**Part 2 — Resume API parameter**  
Optional `resume_from_execution_id: int` and `start_from_step: int` (ge=2) added to `ExecutionStartRequest`. When both are supplied:
1. `validate_resume_point()` in `resume_guard.py` runs **before** the execution is queued — raises HTTP 422 immediately for invalid requests.
2. Resume params are stored in `trigger_details` JSON alongside profile data.
3. `queue_manager.py` extracts and passes them through to `execute_test()`.
4. In `execute_test()`: snapshot for step `start_from_step - 1` is loaded; `_apply_resume_snapshot()` injects cookies + localStorage + sessionStorage and navigates to the stored `page_url`; `_create_skip_records()` writes SKIP rows for steps 1..start_from_step-1; the step loop starts at `idx = start_from_step`.

**Stateful step guard (`resume_guard.py`):** `validate_resume_point()` queries `TestExecutionStep` records for `resume_from_execution_id` and raises HTTP 422 when any prior step has result `FAIL` or `ERROR`.

**OTP guard:** `validate_resume_point()` also calls `is_otp_step()` on each step in the skipped range and raises HTTP 422 if any step is an OTP step.

**UI placement:** Each failed/error step row in `ExecutionProgressPage` now shows a **"↩ Re-run from here"** button (`ReRunFromStepButton` component). Clicking opens a confirmation dialog showing the source execution ID and step; confirming calls `executionService.startExecution()` with `resume_from_execution_id` + `start_from_step` and navigates to the new execution's progress page.

**Files Changed (Actual):**

| File | Change |
|------|--------|
| `backend/app/models/test_execution.py` | New `StepSessionSnapshot` ORM model: `id`, `execution_id`, `step_number`, `page_url`, `session_data` TEXT (JSON), `created_at`; no FK constraint so snapshots from prior completed runs remain accessible |
| `backend/migrations/add_step_session_snapshots_table.py` | New DB migration — run ✅ |
| `backend/app/crud/step_session_snapshot.py` | New: `save_step_session_snapshot()`, `get_step_session_snapshot()` |
| `backend/app/services/resume_guard.py` | New: `validate_resume_point()` — HTTP 422 for failed prior steps or OTP steps in skipped range |
| `backend/app/schemas/test_execution.py` | `ExecutionStartRequest`: `resume_from_execution_id: Optional[int]`, `start_from_step: Optional[int]` (ge=2) |
| `backend/app/services/execution_service.py` | Import `save_step_session_snapshot` / `get_step_session_snapshot`; new helpers `_save_step_snapshot()`, `_apply_resume_snapshot()`, `_create_skip_records()`; `execute_test()` accepts + processes resume params; `_save_step_snapshot()` called after each passing step |
| `backend/app/crud/test_execution.py` | `complete_execution()`: added `skipped_steps` parameter |
| `backend/app/api/v1/endpoints/executions.py` | Import `validate_resume_point`; run guard before queue entry; store resume params in `trigger_details` |
| `backend/app/services/queue_manager.py` | Extract `resume_from_execution_id` + `start_from_step` from `trigger_details`; pass to `execute_test()` |
| `backend/tests/unit/test_resume_execution.py` | 20 unit tests: model columns, schema fields, CRUD helpers, `validate_resume_point` guards, `_save_step_snapshot`, `_apply_resume_snapshot`, `_create_skip_records` |
| `backend/tests/integration/test_resume_e2e.py` | 13 integration tests: CRUD with real SQLite, snapshot save/retrieve, `_apply_resume_snapshot` inject cycle, guard integration, SKIP record counts |
| `frontend/src/types/execution.ts` | `ExecutionStartRequest`: `resume_from_execution_id?: number`, `start_from_step?: number` |
| `frontend/src/components/execution/ReRunFromStepButton.tsx` | New: renders only on fail/error steps — confirmation dialog with source execution + step details; calls `executionService.startExecution()` with resume params; navigates to new execution on success; shows error state on API failure |
| `frontend/src/pages/ExecutionProgressPage.tsx` | `StepCard` wires `ReRunFromStepButton` per step; passes `testCaseId`, `baseUrl`, `browser`, `environment` |
| `frontend/src/components/__tests__/ReRunFromStepButton.test.tsx` | 10 frontend tests: visibility (fail/error shows, pass/skip hides), dialog open/close, cancel does not call API, API called with correct resume params, error state rendered |
...
**Feature A total: 8 points / ~2 days (COMPLETE ✅) · Feature B total: 12 points / 3 days (COMPLETE ✅)**

**Sprint 10.12 total: 20 points / 5 days (COMPLETE ✅)**
...
- **ADR-002-44**: Re-Run from Failed Step — `step_session_snapshots` table, `StepSessionSnapshot` ORM, `resume_guard.py` stateful + OTP guards, `_save_step_snapshot` / `_apply_resume_snapshot` / `_create_skip_records` helpers, `ReRunFromStepButton` frontend component — ✅ **Recorded** in `documentation/ADR-002-test-execution-engine.md`
...
**Feature B (✅ COMPLETE — May 14, 2026)**
- [x] `step_session_snapshots` table persists `(execution_id, step_number, page_url, session_data)` after each passing step
- [x] `POST /tests/{id}/run` accepts `resume_from_execution_id` + `start_from_step`
- [x] Skipped steps recorded as `ExecutionResult.SKIP` with description `"(skipped — resumed from step N of execution X)"`
- [x] Stateful guard: HTTP 422 when source execution has a failing step before the resume point
- [x] OTP guard: HTTP 422 when resume would skip an OTP step
- [x] "↩ Re-run from here" button on each failed/error step row in `ExecutionProgressPage`
- [x] Confirmation dialog shows source execution ID and step number
- [x] 33 backend tests pass (20 unit + 13 integration); 10 frontend tests; 245 total frontend tests — no regression
- [x] ADR-002-44 recorded in `documentation/ADR-002-test-execution-engine.md`
...
| Session snapshot storage growth | Add 30-day TTL cleanup job for `step_session_snapshots`; snapshot per step is ~5–50 KB | 🔄 Future sprint |
...
**Post-release hardening (June 2, 2026 — 3 commits on `fix/imap-model`):** Sprint 10.13 has been extended to support arbitrary OpenAI-compatible vLLM deployments. Three commits (961e16e → 7929b05 → 63b7087) added per-user custom model name, custom endpoint URL, and optional bearer token, propagated across UniversalLLM, the 3-tier execution engine, all 3 Python Stagehand init paths, and the TypeScript `sessionManager.ts`.
...
| **10.13-5** *(commit 961e16e)* Add `local_vllm_custom_model` + `local_vllm_custom_endpoint` columns to `UserSetting` ORM + Pydantic schemas; extend `get_provider_config()` to forward `local_vllm_custom_endpoint`; add Settings page UI fields; update all 4 agents, `test_generation.py`, `orchestration_service.py`, `root_cause_analysis_service.py`, `UniversalLLMService`, `client_factory.py`, `LocalVllmClient` (new `endpoint` constructor param + `_resolve_endpoint` override priority); add `migrate_vllm_custom_model.py` standalone script | `backend/app/models/user_settings.py`, `backend/app/schemas/user_settings.py`, `backend/app/services/user_settings_service.py`, `backend/app/services/universal_llm.py`, `backend/llm/client_factory.py`, `backend/llm/local_vllm_client.py`, `backend/migrate_vllm_custom_model.py`, `frontend/src/pages/SettingsPage.tsx`, `frontend/src/types/api.ts`, agents × 4 | 5 | ✅ |
| **10.13-6** *(commit 7929b05)* Add `backend/migrations/add_local_vllm_custom_model.py` — proper migration runner migration for `local_vllm_custom_model` + `local_vllm_custom_endpoint` columns | `backend/migrations/add_local_vllm_custom_model.py` | 1 | ✅ |
| **10.13-7** *(commit 63b7087)* Add `local_vllm_api_key` column + schema field; add `add_local_vllm_api_key.py` migration; extend `get_provider_config()` to return `api_key`; replace manual `user_ai_config` dict build in `execution_service.py` with `user_settings_service.get_provider_config()`; forward `local_vllm_custom_endpoint` + `api_key` to RCA call; update all 3 Stagehand Python init paths (normal / CDP / persistent) to read `api_key` from `user_config` and pass `model_client_options={"api_base": local_endpoint}`; add `local_vllm` branch + endpoint map + `openai/<model>` naming in TypeScript `sessionManager.ts`; add `local_vllm_custom_endpoint` to `SessionConfig.user_config` TypeScript type; add password-type API Token input in Settings UI | `backend/app/models/user_settings.py`, `backend/app/schemas/user_settings.py`, `backend/migrations/add_local_vllm_api_key.py`, `backend/app/services/user_settings_service.py`, `backend/app/services/execution_service.py`, `backend/app/services/stagehand_service.py`, `backend/app/services/root_cause_analysis_service.py`, `backend/app/services/orchestration_service.py`, `backend/llm/client_factory.py`, `backend/app/services/universal_llm.py`, `backend/app/services/test_generation.py`, `backend/agents/*` × 4, `backend/app/api/v1/endpoints/settings.py`, `frontend/src/pages/SettingsPage.tsx`, `frontend/src/types/api.ts`, `stagehand-service/src/services/sessionManager.ts`, `stagehand-service/src/types/index.ts` | 5 | ✅ |

**Sprint 10.13 total:** original rollout 6 points / 1.5 days (COMPLETE ✅ May 15, 2026) + June 2 hardening 11 points / 2 days across 3 commits (COMPLETE ✅ Jun 2, 2026; 20 files changed, 452 net insertions)

#### Architecture notes

- No API key is required for the original internal vLLM deployments by default; `LOCAL_VLLM_API_KEY` env var still falls back to `"local"`. The June 2 hardening added a per-user `local_vllm_api_key` DB column so auth-required OpenAI-compatible gateways (e.g. `http://192.168.206.164:1235/v1` with token `1235`) are handled without env changes.
- `LocalVllmClient._resolve_endpoint()` now accepts an explicit `override` argument; resolution priority is: (1) caller override, (2) per-model env var, (3) hardcoded default, (4) `LOCAL_VLLM_CUSTOM_ENDPOINT` env var, (5) DeepSeek fallback.
- `execution_service.py` no longer rebuilds a bare 4-key `user_ai_config` dict from raw ORM fields; it calls `user_settings_service.get_provider_config()` so custom model, custom endpoint, thinking flag, and per-user API key are all present when `ThreeTierExecutionService` and RCA receive the config.
- All 3 Stagehand Python init paths (normal, CDP, persistent) now: read `api_key` from `user_config` (falls back to env), apply custom endpoint from `user_config["local_vllm_custom_endpoint"]`, and pass `model_client_options={"api_base": local_endpoint}` into `StagehandConfig`. The `model_client_options` field is the authoritative LiteLLM routing parameter for Tier 2 `observe()`.
- TypeScript `sessionManager.ts` was aligned: new `local_vllm` branch reads `LOCAL_VLLM_API_KEY` env var, builds the same 3-model endpoint map, reads `config.user_config?.local_vllm_custom_endpoint` as override, and prefixes the model name with `openai/` so LiteLLM routes correctly. `SessionConfig.user_config` now includes `local_vllm_custom_endpoint`.
- `local_vllm` remains available in all per-agent model dropdowns via the existing `/api/v1/settings/providers` endpoint — no schema changes needed.

#### June 2 validation

- `user_settings_service.get_provider_config(db, user_id=1, scope="execution")` returns `{provider: local_vllm, model: Qwen3.6-35B-A3B-MLX-8bit, api_key: 1235, local_vllm_custom_endpoint: http://192.168.206.164:1235/v1, ...}` — confirmed live.
- `UniversalLLMService.chat_completion(provider='local_vllm', model='Qwen3.6-35B-A3B-MLX-8bit', custom_endpoint='http://192.168.206.164:1235/v1', api_key='1235')` returned a valid response — confirmed live.
- Full execution-level smoke test through Tier 2 Stagehand `observe()` remains the final operational check.
...
- [x] Settings page shows custom model name + endpoint URL + API Token (password input) fields for `local_vllm`
- [x] `local_vllm_custom_model`, `local_vllm_custom_endpoint`, `local_vllm_api_key` DB columns added via dedicated migrations (`add_local_vllm_custom_model.py`, `add_local_vllm_api_key.py`)
- [x] `get_provider_config()` returns `api_key`, `local_vllm_custom_endpoint`, and resolved model name for all execution paths
- [x] `execution_service.py` uses `get_provider_config()` instead of a bare 4-key manual dict
- [x] All 3 Python Stagehand init paths pass `model_client_options={"api_base": local_endpoint}` and read `api_key` from resolved config
- [x] TypeScript `sessionManager.ts` handles `local_vllm` provider: env-key lookup, 3-model endpoint map, `user_config.local_vllm_custom_endpoint` override, `openai/<model>` naming
- [ ] End-to-end smoke test: run one full execution with `Qwen3.6-35B-A3B-MLX-8bit` through Tier 2 Stagehand `observe()`
...
**June 2 compatibility note:** The local vLLM hardening work added custom-model support for auth-required OpenAI-compatible endpoints such as `Qwen3.6-35B-A3B-MLX-8bit` at `http://192.168.206.164:1235/v1`. Thinking-mode support remains intentionally gated to the known-capable internal model `RedHatAI/Qwen3.6-35B-A3B-NVFP4`; custom models do not automatically inherit the flag unless explicitly added to `thinking_capable_models`.
...
**Sprint 10.15 total: 11 points / ~2.5 days (COMPLETE ✅)**
...
**New step action schema:**
```json
{
  "action": "verify_screenshot",
  "instruction": "Verify the 5G 100GB plan is selected at HK$188/month with the voucher discount applied",
  "expected_items": ["5G 100GB", "HK$188", "voucher"],
  "screenshot_region": "viewport"
}
```
...
### ✅ Sprint 10.18: Developer B — Qwen3.6-35B-A3B-MLX-8bit: Always-Off Thinking Override (June 4, 2026) — COMPLETE
...
- [x] `Qwen3.6-35B-A3B-MLX-8bit` appears in the Settings page Local vLLM model dropdown
- [x] Selecting `Qwen3.6-35B-A3B-MLX-8bit` makes the "Enable Thinking Mode" toggle **available** (not greyed-out)
- [x] With toggle **off** (default): request payload contains `chat_template_kwargs: {"enable_thinking": false}` — thinking is explicitly suppressed
- [x] With toggle **on**: request payload contains `chat_template_kwargs: {"enable_thinking": true}` — thinking is enabled
- [x] `DeepSeek-V4-Flash-4bit` and `openai/gpt-oss-20b` are never sent `chat_template_kwargs` (non-capable, behaviour unchanged)
...
|-------------------|------|
| Model dropdowns populated exclusively from `PROVIDER_CONFIGS` | New model = code change + deploy |
| `__custom__` sentinel + endpoint fields only on `local_vllm` | OpenRouter/Azure/Google/Cerebras users blocked from unlisted models |
| Per-agent slots (`AgentModelConfig.tsx`) use hardcoded `provider.models` only | Agent workflow cannot use custom models even when gen/exec can (for vLLM) |
| `get_agent_config()` does not resolve `__custom__` sentinel | Custom vLLM selection breaks in agent workflow context |
| `generation_model` / `execution_model` columns are `VARCHAR(100)` | Long OpenRouter model IDs may truncate |

---

#### Recommended Architecture — Three-Layer Model Catalogue

```
┌─────────────────────────────────────────────────────────────────┐
│  Layer 1: Curated defaults (PROVIDER_CONFIGS — unchanged role)  │
│  Team-maintained: recommended, is_free, thinking_capable      │
└────────────────────────────┬────────────────────────────────────┘
                             │ merged at API response time
┌────────────────────────────▼────────────────────────────────────┐
│  Layer 2: Per-user registry (custom_models JSON on user_settings)│
│  { "openrouter": ["my/vendor-model:free"],                      │
│    "azure": [{"id":"gpt-5.3","endpoint":"https://...","api_version":"..."}], │
│    "local_vllm": [{"id":"MyModel","endpoint":"http://...","api_key":"..."}] }│
└────────────────────────────┬────────────────────────────────────┘
                             │ surfaced in UI as "⭐ My Models" optgroup
┌────────────────────────────▼────────────────────────────────────┐
│  Layer 3: Ad-hoc entry ("⚙ Custom model..." → free-text input)  │
│  On save: model ID stored in slot field + appended to registry   │
└─────────────────────────────────────────────────────────────────┘
```

**Security constraints (unchanged):**
- Provider API keys (`GOOGLE_API_KEY`, `OPENROUTER_API_KEY`, `AZURE_OPENAI_API_KEY`, etc.) remain in `.env` only.
- Per-user `local_vllm_api_key` (Sprint 10.13) remains the only user-stored credential; optional per-model vLLM token in registry follows same pattern.
- Custom model IDs are validated server-side; no arbitrary URL fetch from user input except whitelisted `local_vllm` / Azure endpoint fields.

**Out of scope (defer to future sprint):**
- Dynamic OpenRouter model list fetch from `openrouter.ai/api/v1/models` (optional "Refresh models" button).
- Admin/global model catalogue shared across all users (per-user only in this sprint).
...
| Dependency | Reason |
|------------|--------|
| Sprint 10.5 | `model_options` + optgroup UI pattern |
| Sprint 10.6 | Per-agent model slots must receive same custom-model support |
| Sprint 10.13 | Existing `__custom__` sentinel + vLLM endpoint/token pattern to generalize |
| Sprint 10.9 | Azure per-model endpoint override pattern to extend for user-defined deployments |
...
| Risk | Likelihood | Impact | Mitigation |
|------|-----------|--------|------------|
| Custom Azure endpoint misconfiguration causes silent routing to wrong resource | Medium | High | Validate endpoint URL format; require explicit endpoint for Azure models not in curated list; log resolved endpoint at call time |
| `SettingsPage.tsx` grows further despite extraction | Medium | Low | Extract `ModelSelectWithCustom` + `CustomModelsPanel` before integrating; target removing ~150 lines of duplicated dropdown logic |
| Per-agent `get_agent_config()` missing custom resolution breaks agent workflow | High | High | Explicit test: agent slot with custom OpenRouter model used in orchestration run |
| Long OpenRouter model IDs exceed `VARCHAR(100)` | Medium | Medium | Migration bumps to `VARCHAR(200)` in Phase 1 |
| User saves invalid/nonexistent model ID | High | Low | Runtime error surfaced in execution logs; optional future "Test connection" button out of scope |

---

#### Relationship to Prior Sprints

This sprint **supersedes the deploy-for-new-model pattern** used in:
- Sprint 10.5 (OpenRouter free model list expansion — curated layer remains, but users can add models beyond the list)
...
#### Recommendation: API Polling — Not Headless Browser

| Approach | Verdict | Reason |
|---|---|---|
| **Headless browser** to read OTP from a portal/UI | ❌ **Rejected** | Second Playwright instance competes with the test browser (CPU, memory, CDP ports); 30–60 s navigation overhead; bot detection risk; unnecessary when a direct API exists |
| **Same browser tab** navigates away to fetch OTP | ❌ **Rejected** | Destroys current page state, cookies, and multi-step flow; breaks 3-tier execution mid-test |
| **Manual OTP entry** in test steps | ⚠️ Fallback only | Blocks CI/automation; acceptable when API unreachable |
| **Backend HTTP API poll (`httpx`)** | ✅ **Selected** | Zero impact on test browser; sub-second per poll; same JIT timing model as IMAP; works in headed and headless test runs alike |

**`HEADLESS_BROWSER` is unrelated to OTP retrieval.** It only controls whether the **test** browser window is visible. OTP fetching runs entirely in the Python backend as async HTTP — independent of Playwright visibility mode.
...
3-tier test browser (Playwright / Stagehand — headed or headless per HEADLESS_BROWSER)
    │ user triggers "Send OTP" in preceding step
    │ browser stays on app page — no navigation away
    ▼
Execution loop reaches OTP placeholder step (e.g. "Enter login OTP for 85291234567")
    │
    ▼  JIT — same pattern as Sprint 10.10
...
PreprodOtpService.poll_otp()  — httpx sync GET (verify=resolve_ssl_verify())
    ...
    ▼
newest matching OTP value (e.g. "482019")
    │
    ▼
format_otp_steps("482019")  — reused from email_otp_service.py
    │
    ▼
execution loop fills per-digit boxes via Tier 1/2/3 — browser never left the app
```

**Stale-OTP prevention (mirrors Sprint 10.10 JIT):** Record `poll_start_time = utc_now()` immediately before the first HTTP poll at the OTP step. Only accept API records with `created_at >= poll_start_time - 5s` (grace window). Pick the **newest** matching record. This avoids reusing an OTP from a prior test run in the same hour window.
...
| Priority | Source selection | When used |
|---|---|---|
...
| Credential storage | Env vars + optional Settings override | API is internal preprod diagnostic endpoint; no per-user secrets expected initially |
| Coexistence with IMAP | Router picks API or IMAP per step | Email OTP flows unchanged; Three HK SMS/login OTP uses API |
...
#### Relationship to Sprint 10.10

Sprint 10.10 solved **email-delivered** OTP via IMAP. Sprint 10.21 solves **SMS/app-delivered** OTP on Three HK preprod where QA exposes a diagnostic API. Both share:
- `is_otp_step()` detection
- `format_otp_steps()` per-digit expansion
- JIT expansion at step execution time
- `otp_expanded_end` re-poll guard

The only new piece is the **OTP source router** and **`PreprodOtpService`** HTTP client. No changes to Tier 1/2/3 executors.
