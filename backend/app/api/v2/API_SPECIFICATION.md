...
| `save_flow_recording` | boolean | No | When `true` (default), after observation the server writes `playwright_flow_recording.json`, `flow_steps.json`, and `playwright_step_ir.json` (deterministic locator IR for tooling) under `backend/artifacts/flow_recordings/{workflow_id}/` (gitignored). **Full** `POST /generate-tests` also writes `by_test_case/test_case_{id}.json` manifests linking each stored test case to those files. Paths appear in `observation_result.flow_recording_artifacts`. Set `false` to skip disk writes. Server-wide disable: `FLOW_RECORDINGS_ENABLED=false` in `.env`. Optional directory override: `FLOW_RECORDINGS_DIR`. |
| `max_browser_steps` | integer | No | Optional browser-use step cap (1–500); server default 120 if omitted. |
