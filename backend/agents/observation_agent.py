...
class ObservationAgent(BaseAgent):
    ...
    async def _execute_multi_page_flow_crawling(
        ...
        try:
            ...
            - CRITICAL — "SWITCH NUMBER" MODAL: If a modal dialog titled "Switch Number" appears (showing a list of phone numbers to choose from), this is a confirmation prompt to assign the new SIM number to the account. Do NOT repeatedly click "Switch Number" button — click it ONCE and then wait 5 seconds for the modal to dismiss and the page to progress. If the modal is still visible after one click and a 5-second wait, press Escape key or look for a close/X button to dismiss it, then check whether the checkout has already progressed.
